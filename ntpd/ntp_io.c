...
{
    msyslog(LOG_NOTICE, "Danish move_fd starts");
...
{
        msyslog(LOG_NOTICE, "Danish collect timming starts");
	char buf[256];
...
{
        msyslog(LOG_NOTICE, "Danish interface_dump starts");
...
init_async_notifications()
{
        msyslog(LOG_NOTICE, "Danish init_async_notification");
	struct asyncio_reader *reader;
#ifdef HAVE_RTNETLINK
